...
export default function EditStationPage({ params }: { params: { stationId: string } }) {
  ...
  useEffect(() => {
    ...
    if (!session || session.stationId !== params.stationId) {
      router.push('/')
      return
    }
  ...
  const addVideo = async () => {
    ...
    try {
      ...
      setVideos(updated)
      saveStationVideos(params.stationId, updated)
      setYoutubeUrl('')
    } catch (err) {
  ...
  const saveSettings = () => {
    ...
    updateStation(params.stationId, {
      settings: {
        ...station.settings,
        aboutText,
        showUpNext,
  ...
  return (
    <div className="min-h-screen bg-black text-white py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <Link href={`/stations/${params.stationId}`} className="text-gray-400 mb-8 inline-block hover:text-white">
          ← Back to Station
        </Link>
